...
def contribute(request):
    ...
    if request.method == "POST":
        ...
        owner=global_First_Name+" "+global_Lname
        powner=owner
        pptype=ptype
        ...
        # contr=Contribute(ptype=ptype,psummary=psummary,pdescription=pdescription,products=products,kanalysis=kanalysis,kinsisghts=kinsisghts,owner=owner,datetime_entry=datetime_entry.strftime('%Y-%m-%d %H:%M:%S:%p'))
        # contr.save()
        conn = MongoClient()
        db=conn.Lucid
        ...
        #   "username":username1,          
          ...
          "tags":finaltags,
          "owner":owner,
          "datetime_entry":datetime_entry.strftime('%Y-%m-%d %H:%M:%S'),
        ...
        contri_to_neo(pdescription,psummary,p2, kanalysis,kinsisghts,owner,ptype, uniqueId2,finaltags,bgid,datetime_entry)
...
def contri_to_neo(ppdescription,ppsummary,pproducts, pkanalysis,pkinsisghts,powner,pptype, uniqueId2,finaltags,bugid,datetime_entry):
    ...
    q2='''Merge (kp:knowledge {pdescription: '%s', ptype: '%s', psummary: '%s',id: '%s' , kanalysis:'%s', kinsisghts:'%s', owner:'%s', products:'%s',bugid:'%s',Tags:'%s',datetime_entry:'%s'})
    WITH kp
    UNWIND split('%s',',') AS tag
    MERGE (t:final_Tags {tagname: tag})
    MERGE (kp)-[:belongs_to]->(t)'''%(ppdescription,pptype,ppsummary,uniqueId2,pkanalysis,pkinsisghts,powner,pproducts,bugid,final_Tags[:-1],datetime_entry.strftime('%Y-%m-%d %H:%M:%S'),final_Tags[:-1])
    q1=" match(n) return n "
...
def jira(request):
    conn = mod4.connect("User=knowledgeplatform64@gmail.com;APIToken=yXtVayMItmxakAJWOVko3931 ;Url=https://knowledgeplatform64.atlassian.net")

    global fullname2
    print("fullname2...........................................",fullname2)
    #fullname = "Smriti Misra"
    #print(fullname)
    fullname2 = "Smriti Misra"

    if request.method == 'POST':
        bug_id = request.POST['jiraid']
        c = "SELECT AssigneeDisplayName FROM Issues WHERE Id=?"
        params= [bug_id]
        cc = conn.execute(c,params)
        rs = cc.fetchall()
        # print(str(rs[0]))
        a=""
        if(len(rs)>0):
        ...
        if(a == fullname2):
            ...
            messages.error(request, "Enter correct bug id")
...
def update_data(request):
    ...
    datetime_entry1 = datetime.datetime.now()
    conn = MongoClient()
    db=conn.Lucid
    collection=db.knowledge
    ...
    if request.method=="POST":
        ...
        for i in finaltags:
            tags_string+=i+"," 
        print("This is in update:%s",tags_string)
        db.knowledge.update({'ID':uniqueId},{'ID':uniqueId,'ptype':ptype,'psummary':psummary,'pdescription':pdescription,'products':products,'kanalysis':kanalysis,'kinsisghts':kinsisghts,'owner':global_First_Name+" "+global_Lname,'tags':finaltags,'datetime_entry':datetime_entry1.strftime('%Y-%m-%d %H:%M:%S')})    #update this knowledge in MongoDb
        messages.success(request, "Data Updated Successfully")
        p2=""
        for i in products:
            p2+=i+","
        p2=p2[:-1]
        owner_to_neo=global_First_Name+" "+global_Lname
        update_to_neo(pdescription,psummary,p2, kanalysis,kinsisghts,owner_to_neo,ptype, uniqueId,datetime_entry1.strftime('%Y-%m-%d %H:%M:%S'),finaltags) #update this knowledge in neo4j
    return render(request, "authentication/index.html") 

#This function is used to update data in neo4j when user update any knowledge
def update_to_neo(ppdescription,ppsummary,pproducts, pkanalysis,pkinsisghts,powner,pptype, uniqueId2,datetime_entry1,finaltags):
    #global ppdescription,ppsummary,pproducts, pkanalysis,pkinsisghts,powner,pptype, uniqueId2,finaltags
    final_Tags=""
    for i in finaltags:
        final_Tags+=i+","
    ...
    products: '%s',psummary:"%s",datetime_entry:"%s",Tags:"%s"}
    ''' %( uniqueId2,uniqueId2,powner,ppdescription,pptype,pkanalysis,pkinsisghts,pproducts,ppsummary,datetime_entry1,final_Tags[:-1])
    q33='''MATCH (p {id:'%s'})
    UNWIND split("%s", ',') AS tag
    MERGE (t:final_Tags {tagname: tag})
    MERGE (p)-[:belongs_to]->(t)'''%(uniqueId2,final_Tags[:-1])
    session.run(q33)
    session.run(q3)
